...
export default function AccountSidebar() {
  const [AcountDtat, setAcountDtat] = useState(null);
  const { data: session } = useSession();
  const accessToken = session?.user?.accessToken;

  async function HandleProfileData(currentToken) {
    if (!currentToken) {
      console.warn("No access token available for profile request");
      return;
    }
    try {
      const data = await GetProfileDtat({ token: currentToken });
  ...
  return (
    ...
    >
      ...
      <nav className="mt-4 space-y-1 text-sm">
        ...
        <SidebarLink href="/account/deposit" label="الإيداع" icon={Landmark} />
        <SidebarLink
        ...
        <SidebarInfo label="الحد الأقصى للسداد الشهري" value="0.00 جنيه" />
